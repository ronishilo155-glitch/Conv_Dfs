...
#define CONV_INPUT_ADDR_REG_IDX  (CONV_REGS_BASE_IDX + 0) // SRC_ADDR_IDX
#define CONV_OUTPUT_ADDR_REG_IDX (CONV_REGS_BASE_IDX + 1) // DST_ADDR_IDX
#define CONV_N_SIZE_REG_IDX      (CONV_REGS_BASE_IDX + 2) // N_SIZE_IDX
#define CONV_KERNEL_REG_IDX      (CONV_REGS_BASE_IDX + 3) // KERNEL_REG_IDX
#define CONV_START_REG_IDX       (CONV_REGS_BASE_IDX + 4) // START_REG_IDX
#define CONV_DONE_REG_IDX        (CONV_REGS_BASE_IDX + 5) // DONE_REG_IDX
...
#define CONV_N_SIZE_REG      ((volatile unsigned int *) (XBOX_REGS_BASE_ADDR + (4*CONV_N_SIZE_REG_IDX))) 
#define CONV_KERNEL_REG      ((volatile unsigned int *) (XBOX_REGS_BASE_ADDR + (4*CONV_KERNEL_REG_IDX))) 
...
static void conv2x2_stage_flat_bm(
    ...
    for (r = 0; r < out_size; r++) {
        ...
        for (c = 0; c < out_size; c++) {
            ...
            acc >>= 2;           // Normalize to keep values in 0-255 range
            out[r * out_size + c] = (uint16_t) acc;
        }
    }
}
...
// Assumes T <= 16
// ------------------------------------------------------------
static uint16_t pipeline_process_tile_streaming_bm(
    ...
    uint16_t bufB[16 * 16];
    int i;
    for (i = 0; i < T * T; i++) bufA[i] = (uint16_t) tile_flat[i];
    ...
    for (stage = 0; stage < depth; stage++) {
        ...
        uint16_t* tmp = in; in = out; out = tmp;
    }
...
static void process_full_image_bm(
    ...
    for (tr = 0; tr < out_dim; tr++) {
        for (tc = 0; tc < out_dim; tc++) {
            ...
            uint16_t act = pipeline_process_tile_streaming_bm(
...
//bm_printf("\n[CPU] Configuring Hardware Accelerator...\n");

    // 1. Pack the 4 kernel weights (8-bit each) into a single 32-bit integer
    unsigned char* k_ptr = conv_config_p->kernel_addr;
    unsigned int packed_kernel = 0;
    packed_kernel |= ((unsigned int)k_ptr[3] << 24);
    packed_kernel |= ((unsigned int)k_ptr[2] << 16);
    packed_kernel |= ((unsigned int)k_ptr[1] << 8);
    packed_kernel |= ((unsigned int)k_ptr[0]);

    // 2. Write configuration to Hardware Registers (APB)
    *CONV_INPUT_ADDR_REG  = (unsigned int) conv_config_p->input_addr;
    *CONV_OUTPUT_ADDR_REG = (unsigned int) conv_config_p->output_addr;
    *CONV_N_SIZE_REG      = (unsigned int) conv_config_p->input_width; // N (e.g., 256)
    *CONV_KERNEL_REG      = packed_kernel;

    //bm_printf("[CPU] Accelerator Configured. Sending START command.\n");

    // 3. Trigger Start
    *CONV_START_REG = 1;
    *CONV_START_REG = 0;

    // 4. Poll the Done Register (Wait for Hardware to finish)
    // The hardware will set the LSB of CONV_DONE_REG to 1 when finished
    while(!(*CONV_DONE_REG)) {
        // HW is running... 
        // We can place a small delay or just poll continuously (like the instructor did)
    }

    //bm_printf("[CPU] Received DONE from Hardware Accelerator!\n");

    // Note: The clear-on-read mechanism in Verilog will automatically clear the 
    // DONE register now that we have read it in the while loop.
}
